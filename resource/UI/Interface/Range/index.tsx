...
const RangeMarkerList = styled.div
`
    margin: 0;
    padding: 0 8px;
    display: flex;
...
export const Range: FunctionComponent<CheckboxProps> = ({
    ...
    const leftButtonClickHandler = useCallback((event: MouseEvent<HTMLButtonElement>): void => {
        ...
        Number(value) - Number(step) >= min && onChange(Number(value) - Number(step));
    }, [ min, step, value, onChange ]);

    const rightButtonClickHandler = useCallback((event: MouseEvent<HTMLButtonElement>): void => {
        event.preventDefault();
        Number(value) + Number(step) <= max && onChange(Number(value) + Number(step));
